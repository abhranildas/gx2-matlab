function coeffs=gx2_ruben_coeffs(w,k,lambda,varargin)

% GX2_RUBEN_COEFFS Ruben's series expansion coefficients for the generalized
% chi-square with weights w, degrees of freedom k and non-centralities lambda.
%
% Abhranil Das
% Center for Perceptual Systems, University of Texas at Austin
% Comments, questions, bugs to abhranil.das@utexas.edu
%
% Depends only on (w,k,lambda) and the term-count cap n_ruben -- not on the
% evaluation point x or offset m -- so callers evaluating many points against
% the same (w,k,lambda) (e.g. gx2_dens_deriv's mixed-sign convolution, which
...
%
% Usage:
% coeffs=gx2_ruben_coeffs(w,k,lambda)
% coeffs=gx2_ruben_coeffs(w,k,lambda,'n_ruben',500)
%
% Required inputs:
% w         row vector of weights, all the same sign
% k         row vector of degrees of freedom
% lambda    row vector of non-centralities
%
...
parse(parser,w,k,lambda,varargin{:});
